...
function LoginForm() {
  ...
  const oauthErrorMessage =
    oauthError === "google_login_failed"
      ? "Google sign in could not be completed. Please try again or create an account."
      : "";
  ...
  function validate(): boolean {
    const next = {
      ...
      password: validatePassword(password, "login") ?? undefined,
    };
    setFieldErrors(next);
  ...
  async function handleFormSubmit(e: React.FormEvent) {
    ...
    try {
      const gated = await assertAuthThrottle("login");
      if (!gated.ok) {
                ...
                <form className="space-y-6" onSubmit={handleFormSubmit} noValidate>
                  ...
                  <FormField
                    ...
                    onChange={(e) => {
                      ...
                      if (touched.password) {
                        setFieldErrors((prev) => ({ ...prev, password: validatePassword(e.target.value, "login") ?? undefined }));
                      }
                    }}
                    onBlur={() => {
                      setTouched((prev) => ({ ...prev, password: true }));
                      setFieldErrors((prev) => ({ ...prev, password: validatePassword(password, "login") ?? undefined }));
                    }}
                  />
